...
public class BaseUtils {
   ...
   /* *//**返回参数存在**//*
    public void  assertExitResult(String url,Object oboject,String method,String currentmethod) {
        try {
            Response response = HttpService.exceHttp(url,oboject,method);
            *//** response.body().string() 只能使用一次，再次调用就会关闭 **//*
            JSONObject jsonObject = JSONObject.parseObject(response.body().string());
            JSONObject jsonData = jsonObject.getJSONObject("Data");
            try {
                Assert.assertEquals("2000",jsonObject.get("Code"));
   ...
   /* *//**返回参数不存在**//*
    public void assertNullResult(String url, Object oboject,String method,String currentmethod) {
        try {
            Response response = HttpService.exceHttp(url,oboject,method);
            *//** response.body().string() 只能使用一次，再次调用就会关闭 **//*
            JSONObject jsonObject = JSONObject.parseObject(response.body().string());
    ...
    /**返回异常信息**/
   /* public void assertErrorResult(String url,Object oboject,String method,String currentmethod) {
        try {
            Response response = HttpService.exceHttp(url,oboject,method);
            *//** response.body().string() 只能使用一次，再次调用就会关闭 **//*
            JSONObject jsonObject = JSONObject.parseObject(response.body().string());
