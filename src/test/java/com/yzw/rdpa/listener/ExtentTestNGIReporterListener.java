...
public class ExtentTestNGIReporterListener implements IReporter {
    ...
    private void init() {
        ...
        htmlReporter.config().setDocumentTitle(ReportsUtil.getReportName());
        htmlReporter.config().setReportName(ReportsUtil.getReportName());
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                while (logIterator.hasNext()){
                    ...
                    if(details.contains(ReportsUtil.getSpiltTimeAndMsg())){
                        String time = details.split(ReportsUtil.getSpiltTimeAndMsg())[0];
                        log.setTimestamp(getTime(Long.valueOf(time)));
                        log.setDetails(details.substring(time.length()+ReportsUtil.getSpiltTimeAndMsg().length()));
                    }else{
